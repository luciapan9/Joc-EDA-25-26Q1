...
struct PLAYER_NAME : public Player {
  ...
  pair<Dir,int> bfs_to_target(Pos start, Pos target) {
    ...
    while (not Q.empty()) {
      ...
      for (Dir dir : Dirs) {
        ...
        // Obstáculos: Aliados y Enemigos son muros
  ...
  void move_soldiers() {
    ...
    for (auto const& [id_box, pos_box] : box_positions) {
      ...
      if (!se_movio) {
        Dir d = movimiento_aleatorio_seguro(u.pos);
        move(id, d);
        moved_soldiers.insert(id);
      }
    }
  ...
  void move_profesor() {
    ...
    if (not moved) {
      pair<Dir,int> bfs_money = bfs_professor(p,dinero,id_box,2);
    ...
    if (not moved) {
      pair<Dir,int> bfs_poison = bfs_professor(p,kit_veneno,id_box,3);
    ...
    if (not moved) {
      pair<Dir,int> bfs_salud = bfs_professor(p,kit_salud,id_box,2);
